...
set_token_matcher = on_command("naga-set-cookies", priority=4, block=True)
...
async def naga_set_cookies(matcher: Matcher, cmd_args=CommandArg()):
    ...
    if "csrftoken" not in cookies or "naga-report-session-id" not in cookies:
        await matcher.finish(f"格式：{default_cmd_start}naga-set-cookies csrftoken=xxxxxxx; naga-report-session-id=xxxxxxx")
